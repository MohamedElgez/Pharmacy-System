...
import {useGetReturnSalesQuery} from "../../services/medicines/medicines";
...
const AllReturnSales = () => {
   const [page, setPage] = useState('1')

  const { data, isLoading, isError, isSuccess } = useGetReturnSalesQuery(page);
const numberOfPage = Array.from(Array(data?.last_page).keys()); 

  const salees = data?.data
 useEffect(()=>{
  dataTable()
}, [isSuccess])
            ...
            <div className="card card-default">
              <div className="card-header">
                <h3 className="card-title">
                  <Link to={"/AddReturnSale"}>
                    <i className="fa fa-bars"></i>
                    <span className="ms-2">{"Add Return Sale"}</span>
                  </Link>
                </h3>
              </div>
