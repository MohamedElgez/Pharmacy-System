...
const Expiration = () => {
  ...
  const reportsExpiration = useReportsExpirationQuery(page);

  const numberOfPage = Array.from(Array(reportsExpiration?.last_page).keys()); 

  console.log(reportsExpiration);
  
  const items = reportsExpiration?.data?.Expire_item?.data
console.log(items);
  useEffect(()=>{
  dataTable()
